# Generated by Django 4.1 on 2023-07-30 20:28

import django.core.validators
from django.db import migrations, models
import django.db.models.deletion
import fills.models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='ColumnRule',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('column_name', models.CharField(max_length=3, validators=[django.core.validators.RegexValidator(message='单元格名称必须是大写英文字母', regex='^[A-Z]+$')], verbose_name='单元格列')),
                ('column_type', models.CharField(choices=[('string', '字符串'), ('number', '数值'), ('boolean', '布尔值')], max_length=64, verbose_name='单元格数据类型')),
                ('associated_of', models.BooleanField(default=False, verbose_name='是否需要外部数据集')),
            ],
            options={
                'verbose_name': '列规则',
                'verbose_name_plural': '列规则',
                'db_table': 'column_rule',
            },
        ),
        migrations.CreateModel(
            name='DataSet',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('username', models.CharField(max_length=255, verbose_name='用户名')),
                ('description', models.TextField(verbose_name='描述')),
                ('data_type', models.CharField(choices=[('string', '字符串'), ('dict', '字典')], max_length=255, verbose_name='数据类型（字典或字符串）')),
            ],
            options={
                'verbose_name': '数据集',
                'verbose_name_plural': '数据集',
                'db_table': 'data_set',
            },
        ),
        migrations.CreateModel(
            name='FileRecord',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('requirement_id', models.BigIntegerField(verbose_name='关联填充要求')),
                ('username', models.CharField(max_length=255, verbose_name='用户名')),
                ('file_id', models.CharField(max_length=255, unique=True, verbose_name='对象存储ID')),
                ('filename', models.CharField(max_length=255, verbose_name='原始文件名')),
            ],
            options={
                'verbose_name': '文件记录',
                'verbose_name_plural': '文件记录',
                'db_table': 'file_record',
            },
        ),
        migrations.CreateModel(
            name='FillingRequirement',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('username', models.CharField(max_length=255, verbose_name='用户名')),
                ('remark', models.TextField(blank=True, verbose_name='备注')),
                ('file_id', models.CharField(max_length=255, unique=True, verbose_name='对象存储ID')),
                ('original_filename', models.CharField(max_length=255, validators=[django.core.validators.RegexValidator(message='必须为.xlsx格式的excel文件', regex='.xlsx$')], verbose_name='原始文件名')),
                ('start_line', models.PositiveIntegerField(validators=[django.core.validators.MinValueValidator(1, message='起始行在1到42之间'), django.core.validators.MaxValueValidator(42, message='起始行在1到42之间')], verbose_name='起始行')),
                ('line_number', models.PositiveIntegerField(validators=[django.core.validators.MinValueValidator(1, message='支持填充行数在1到200之间'), django.core.validators.MaxValueValidator(200, message='支持填充行数在1到200之间')], verbose_name='结束行')),
            ],
            options={
                'verbose_name': '填充要求',
                'verbose_name_plural': '填充要求',
                'db_table': 'filling_requirement',
            },
        ),
        migrations.CreateModel(
            name='GenerateRule',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('rule_name', models.CharField(max_length=128, unique=True, verbose_name='规则名')),
                ('function_name', models.CharField(max_length=255, unique=True, verbose_name='函数名')),
                ('fill_order', models.PositiveIntegerField(verbose_name='填入顺序')),
                ('description', models.TextField(verbose_name='功能描述')),
            ],
            options={
                'verbose_name': '生成规则',
                'verbose_name_plural': '生成规则',
                'db_table': 'generate_rule',
            },
        ),
        migrations.CreateModel(
            name='GenerateRuleParameter',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('name', models.CharField(max_length=255, verbose_name='参数名')),
                ('data_type', models.CharField(choices=[('string', '字符串'), ('number', '数值'), ('boolean', '布尔值')], max_length=255, verbose_name='数据类型')),
                ('description', models.CharField(max_length=512, verbose_name='描述')),
                ('hints', models.CharField(blank=True, max_length=512, verbose_name='输入提示')),
                ('required', models.BooleanField(verbose_name='是否必填')),
                ('default_value', models.CharField(blank=True, max_length=255, verbose_name='默认值')),
                ('need_outside_data', models.BooleanField(verbose_name='是否需要外部数据')),
                ('rule', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.generaterule', verbose_name='关联规则')),
            ],
            options={
                'verbose_name': '生成规则参数',
                'verbose_name_plural': '生成规则参数',
                'db_table': 'generate_rule_parameter',
            },
        ),
        migrations.CreateModel(
            name='DataSetValue',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('item', models.TextField(verbose_name='数据项')),
                ('data_type', models.CharField(choices=[('string', '字符串'), ('dict', '字典')], max_length=255, verbose_name='数据类型（字典或字符串）')),
                ('data_set', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.dataset', verbose_name='关联数据集')),
            ],
            options={
                'verbose_name': '数据集数据',
                'verbose_name_plural': '数据集数据',
                'db_table': 'data_set_value',
            },
        ),
        migrations.CreateModel(
            name='DataSetDefine',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('name', models.CharField(max_length=255, validators=[django.core.validators.RegexValidator(message='属性命名只能为大小写英文字母', regex='^[a-zA-Z]+$')], verbose_name='属性名')),
                ('data_type', models.CharField(choices=[('string', '字符串'), ('number', '数值'), ('boolean', '布尔值')], max_length=255, verbose_name='数据类型')),
                ('data_set', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.dataset', verbose_name='关联数据集')),
            ],
            options={
                'verbose_name': '数据集定义',
                'verbose_name_plural': '数据集定义',
                'db_table': 'data_set_define',
            },
        ),
        migrations.CreateModel(
            name='DataSetBind',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('column_name', models.CharField(max_length=3, verbose_name='单元格列名')),
                ('data_name', models.CharField(max_length=255, verbose_name='关联属性名')),
                ('column_rule', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.columnrule', verbose_name='关联列规则')),
                ('data_set', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.dataset', verbose_name='关联数据集')),
            ],
            options={
                'verbose_name': '数据集绑定列',
                'verbose_name_plural': '数据集绑定列',
                'db_table': 'data_set_bind',
            },
        ),
        migrations.CreateModel(
            name='DataParameter',
            fields=[
                ('id', models.BigIntegerField(default=fills.models.gen_id, editable=False, primary_key=True, serialize=False)),
                ('created_at', models.DateTimeField(auto_now_add=True, verbose_name='创建时间')),
                ('updated_at', models.DateTimeField(auto_now=True, verbose_name='修改时间')),
                ('name', models.CharField(max_length=255, verbose_name='参数名')),
                ('value', models.CharField(blank=True, default=str, max_length=512, verbose_name='参数值')),
                ('data_set_id', models.BigIntegerField(default=None, null=True, verbose_name='关联数据集')),
                ('column_rule', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.columnrule', verbose_name='关联列规则')),
                ('param_rule', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.generateruleparameter', verbose_name='关联参数')),
            ],
            options={
                'verbose_name': '列规则参数',
                'verbose_name_plural': '列规则参数',
                'db_table': 'data_parameter',
            },
        ),
        migrations.AddField(
            model_name='columnrule',
            name='requirement',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.fillingrequirement', verbose_name='关联填充要求'),
        ),
        migrations.AddField(
            model_name='columnrule',
            name='rule',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='fills.generaterule', verbose_name='关联规则'),
        ),
        migrations.AddConstraint(
            model_name='generateruleparameter',
            constraint=models.UniqueConstraint(fields=('rule_id', 'name'), name='unique_param_name'),
        ),
        migrations.AddConstraint(
            model_name='datasetdefine',
            constraint=models.UniqueConstraint(fields=('data_set_id', 'name'), name='unique_data_attr_name'),
        ),
        migrations.AddConstraint(
            model_name='datasetbind',
            constraint=models.UniqueConstraint(fields=('data_set_id', 'column_rule_id', 'column_name'), name='unique_column_bind'),
        ),
        migrations.AddConstraint(
            model_name='columnrule',
            constraint=models.UniqueConstraint(fields=('requirement_id', 'column_name'), name='unique_column_name'),
        ),
    ]
